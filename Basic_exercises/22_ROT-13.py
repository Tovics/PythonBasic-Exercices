...
Once you're done, you will be able to read the following secret message: Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!
...
def rot_13(cipher_table, some_string):
    result = ""
    print(some_string)
    for word in re.findall('[a-zA-Z0-9?!]+', some_string):
            for j in range(len(word)):
                if word[j] in cipher_table:
                    result = result + cipher_table[word[j]]
                else:
...
def main():
    ...
    check = "Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!"
    print(rot_13(cipher, check))
